...
The companion gist includes a small, single-file NumPy implementation,
[`fv_toy.py`](https://gist.github.com/eNipu/023d3537f1c2464617c5de43b96dcdfe#file-fv_toy-py), that does everything above with the toy
...
circuit is), we can in principle compute *anything* on encrypted data. Before
the remaining ideas, here is the whole life of a ciphertext on one screen, with
the two resources every operation touches, noise and size, tracked alongside:
...
bootstrap(ct)       noise: RESET           (when the budget runs low)
     |
     v
decrypt(sk, ct)     correct iff total noise < q/(2t)
```

Three more ideas make it practical:
...
If you want to poke at it, everything here is runnable. The companion code
lives in [one gist](https://gist.github.com/eNipu/023d3537f1c2464617c5de43b96dcdfe): `fv_toy.py` is the complete toy FV scheme (keygen,
encrypt, decrypt, add, multiply) in a single file of NumPy, and
`noise_plots.py` regenerates the noise figures. The Manim sources for every
animation and diagram are in the site's
[repository](https://github.com/eNipu/eNipu.github.io/tree/master/blog/manim/scenes).
